...
# 1. Intercept Render's HTTP HEAD/GET health check probes cleanly at the protocol level
class HealthCheckServerConnection(ServerConnection):
    def respond_to_invalid_request(self, exception):
        if "got HEAD" in str(exception) or "got GET" in str(exception):
            return (
                http.HTTPStatus.OK,
...
async def main():
    # 2. Bind directly to port 8765 using our custom health-checking connection layer
    async with serve(
        handler, 
        "0.0.0.0", 
        8765, 
        create_connection=HealthCheckServerConnection
    ):
